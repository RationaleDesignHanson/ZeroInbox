...
async function syncPhase(phase: number) {
  switch (phase) {
    case 1:
...
async function phase1_CriticalColorFixes() {
  ...
  await createColorStyle(
    'Archetype/Mail/Gradient Start',
    iOSTokens.colors.mailGradient.start
  );

  await createColorStyle(
    'Archetype/Mail/Gradient End',
    iOSTokens.colors.mailGradient.end
  );

  await createColorStyle(
    'Archetype/Ads/Gradient Start',
    iOSTokens.colors.adsGradient.start
  );

  await createColorStyle(
    'Archetype/Ads/Gradient End',
    iOSTokens.colors.adsGradient.end
  );
...
async function phase6_ShadowEffects() {
  ...
  for (const [name, shadow] of Object.entries(iOSTokens.shadows)) {
    await createEffectStyle(
      `Shadow/${capitalize(name)}`,
      shadow
    );
  }
...
async function createColorStyle(name: string, hex: string) {
...
async function createTextStyle(name: string, fontSize: number, fontWeight: number) {
...
async function createEffectStyle(name: string, shadow: any) {
