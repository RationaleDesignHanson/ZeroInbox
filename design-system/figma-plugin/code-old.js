...
function resolveToken(value, tokensObj) {
...
function createColorPalette() {
    ...
    primaryColors.fills = [];
    Object.entries(tokens.colors.primary).forEach(([name, data]) => {
        if (name === '$type')
            return;
    ...
    Object.entries(tokens.colors.semantic).forEach(([name, data]) => {
        if (name === '$type')
            return;
        const colorCard = figma.createFrame();
        colorCard.name = name;
    ...
    Object.entries(tokens.colors.opacity).forEach(([name, data]) => {
        if (name === '$type')
            return;
...
function createSpacingScale() {
    ...
    Object.entries(tokens.spacing).forEach(([name, data]) => {
        const row = figma.createFrame();
...
function createSizingTokens() {
    ...
    Object.entries(tokens.sizing).forEach(([name, data]) => {
        const row = figma.createFrame();
        row.name = `Size ${name}`;
        row.layoutMode = 'HORIZONTAL';
        row.itemSpacing = 16;
...
function createBorderRadiusScale() {
    ...
    Object.entries(tokens.borderRadius).forEach(([name, data]) => {
        const row = figma.createFrame();
...
function createArchetypesDisplay() {
    ...
    Object.entries(tokens.archetypes).forEach(([name, data]) => {
        const row = figma.createFrame();
...
function createTypographySpecimen() {
    ...
    Object.entries(tokens.typography.fontWeights).forEach(([name, data]) => {
        ...
        example.fontSize = 15;
        example.fontName = { family: FONT_FAMILY, style: FONT_WEIGHTS[name] };
    ...
    Object.entries(tokens.typography.lineHeights).forEach(([name, data]) => {
    ...
    Object.entries(tokens.typography.fontSizes).forEach(([sizeName, sizeData]) => {
...
function createColorStyles() {
    ...
    Object.entries(tokens.actionPriorities).forEach(([name, data]) => {
        if (name === '$type')
            return;
        const style = figma.createPaintStyle();
...
function createTextStyles() {
    ...
    const textStyles = {};
    Object.entries(tokens.typography.fontSizes).forEach(([size, data]) => {
        if (size === '$type')
            return;
...
function createToastComponent(colorStyles, textStyles) {
...
function createActionCards() {
    ...
    const priorities = [];
    Object.entries(tokens.actionPriorities).forEach(([key, data]) => {
        if (key !== '$type') {
            priorities.push({
