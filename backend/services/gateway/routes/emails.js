...
async function classifyEmail(email) {
  try {
    // Validate email object before sending to classifier
    if (!email || typeof email !== 'object') {
      logger.error('Invalid email object for classification', { email, type: typeof email });
      throw new Error('Email must be a valid object');
    }

    if (!email.subject || !email.from) {
      logger.error('Email missing required fields for classification', {
        hasSubject: !!email.subject,
        hasFrom: !!email.from,
        emailId: email.id
      });
      throw new Error('Email must have subject and from fields');
    }
