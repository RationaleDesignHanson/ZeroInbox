...
function suggestActions(intentId, entities, emailContext = {}) {
  ...
  // SMART PRIORITY BOOSTING: Prefer actions that have all required data
  // - GO_TO actions without a valid URL are deprioritized (boost priority number = lower rank)
  // - Actions with complete data are promoted (lower priority number = higher rank)
  suggestions.forEach(suggestion => {
    // Penalize GO_TO actions with unsubstituted URL templates
    if (suggestion.actionType === 'GO_TO' && suggestion.context.url) {
      if (suggestion.context.url.includes('{') && suggestion.context.url.includes('}')) {
        // URL is an unsubstituted template like "{schedulingUrl}"
        suggestion.priority += 10; // Deprioritize significantly
        suggestion._deprioritized = 'missing_url_data';
        logger.info('⬇️ Deprioritized GO_TO action with unsubstituted URL', {
          actionId: suggestion.actionId,
          originalPriority: suggestion.priority - 10,
          newPriority: suggestion.priority,
          urlTemplate: suggestion.context.url
        });
      }
    }
  });
