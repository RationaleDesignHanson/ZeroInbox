console.log('Loading rules-engine from:', __filename);
/**
 * Rules Engine
...
function suggestActions(intentId, entities, emailContext = {}) {
  ...
  logger.info('🔍 Potential actions for intent', {
    intentId,
    potentialActionIds: potentialActions.map(a => a.actionId),
    potentialActionsCount: potentialActions.length
  });
  ...
  logger.info('✅ Valid actions after filtering', {
    intentId,
    validActionIds: validActions.map(a => a.actionId),
    validActionsCount: validActions.length,
    entitiesAvailable: Object.keys(entities)
  });
  ...
  logger.info('📋 Built action suggestions', {
    intentId,
    suggestionIds: suggestions.map(s => s.actionId),
    suggestionsCount: suggestions.length,
    suggestionsWithUrl: suggestions.filter(s => s.context && s.context.url).map(s => s.actionId)
  });

  // Sort by priority (lower number = higher priority)
  suggestions.sort((a, b) => a.priority - b.priority);

  logger.info('🔢 Sorted suggestions by priority', {
    intentId,
    sortedIds: suggestions.map(s => `${s.actionId}(p${s.priority})`)
  });
