...
function extractTrackingEntities(text) {
  ...
  const trackingPatterns = [
    ...
    { pattern: /\b(1Z[A-Z0-9]{16})\b/i, carrier: 'UPS' },
    // FedEx: 12-14 digits
    { pattern: /\b(\d{12,14})\b/, carrier: 'FedEx' },
    // USPS: 20-22 digits
    { pattern: /\b(\d{20,22})\b/, carrier: 'USPS' },
    // Generic tracking
    { pattern: /tracking\s*#?\s*:?\s*([A-Z0-9]{10,})/i, carrier: null }
  ];
...
function extractPaymentEntities(text) {
  ...
  for (const pattern of amountDuePatterns) {
    const match = text.match(pattern);
    if (match && match[1]) {
      entities.amountDue = parseFloat(match[1].replace(/,/g, ''));
      break;
    }
  }
  ...
  for (const pattern of paymentPatterns) {
    const match = text.match(pattern);
    if (match && match[1]) {
      entities.paymentAmount = parseFloat(match[1].replace(/,/g, ''));
      break;
    }
  }
  ...
  // Payment link
  const paymentLinkMatch = text.match(/(https?:\/\/[^\s]+(?:pay|invoice|bill)[^\s]*)/i);
  if (paymentLinkMatch) {
    entities.paymentLink = paymentLinkMatch[1];
  }
