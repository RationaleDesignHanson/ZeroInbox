...
/**
 * In-memory cache for thread metadata
 * Reduces API calls for frequently accessed threads
 */
const threadCache = new Map();
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes
const CACHE_MAX_SIZE = 1000; // Prevent memory bloat
...
/**
 * Get thread metadata from cache or fetch from API
 */
async function getThreadMetadata(gmail, threadId) {
  ...
  if (cached && cached.expires > Date.now()) {
    cacheStats.hits++;
    logger.debug('Thread cache hit', { threadId, hitRate: getCacheHitRate() });
    return cached.data;
  }
  ...
  try {
    ...
    // Store in cache
    setCachedThreadMetadata(threadId, metadata);
...
/**
 * Store thread metadata in cache with TTL
 */
function setCachedThreadMetadata(threadId, metadata) {
  // Evict oldest entries if cache is full
  if (threadCache.size >= CACHE_MAX_SIZE) {
...
function clearExpiredCache() {
  ...
  if (cleared > 0) {
    logger.info('Cleared expired cache entries', { cleared, cacheSize: threadCache.size });
  }
}

// Clear expired cache every 2 minutes
setInterval(clearExpiredCache, 2 * 60 * 1000);

// Log cache stats every 5 minutes
setInterval(() => {
  logger.info('Thread cache statistics', {
...
router.get('/messages', async (req, res) => {
  try {
    ...
    // Cache thread metadata for future requests
    threads.forEach(thread => {
      const threadLength = thread.messages ? thread.messages.length : 1;
      setCachedThreadMetadata(thread.id, {
        threadId: thread.id,
        messageCount: threadLength,
    ...
    logger.info('Fetched emails with cache', {
...
router.get('/threads/:threadId', async (req, res) => {
  try {
    ...
    // Cache thread metadata for future requests
    setCachedThreadMetadata(threadId, {
      threadId: thread.data.id,
      messageCount: threadLength,
      historyId: thread.data.historyId
    });
...
router.get('/search', async (req, res) => {
  try {
    ...
    logger.info('Search completed with cache', {
      userId,
